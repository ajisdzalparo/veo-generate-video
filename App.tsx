...
const App: React.FC = () => {
  ...
  }>({ file: null, previewUrl: null, base64: null, mimeType: null });
    ...
    <div className="min-h-screen bg-gray-900 p-4 sm:p-6 lg:p-8">
      <div className="max-w-7xl mx-auto">
        <header className="text-center mb-10">
          <h1 className="text-4xl sm:text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-indigo-600">VEO Video Generator</h1>
          <p className="mt-4 text-lg text-gray-400">Craft stunning videos from your ideas using the power of AI.</p>
        </header>
            ...
            <form onSubmit={handleSubmit}>
              <div className="space-y-6">
                    ...
                    <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-600 border-dashed rounded-md">
                      <div className="space-y-1 text-center">
                        ...
                        <div className="flex text-sm text-gray-400">
                          <label
                            htmlFor="file-upload"
                            className="relative cursor-pointer bg-gray-900 rounded-md font-medium text-indigo-400 hover:text-indigo-300 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-indigo-500 focus-within:ring-offset-gray-900"
                          >
                ...
                <div>
                  <button
                    type="submit"
                    disabled={!prompt || !apiKey || isLoading}
        ...
        <footer className="mt-10 text-center text-gray-500 text-sm">Developed by Ajis Dzalparo</footer>
      </div>
    </div>
  );
};
...
const RadioButton = ({ value, label, isChecked, onChange, disabled }: { value: string; label: string; isChecked?: boolean; onChange?: () => void; disabled?: boolean }) => (
  <label
    className={`w-full text-center py-2 px-4 rounded cursor-pointer text-sm font-medium transition-colors ${isChecked ? "bg-indigo-600 text-white shadow" : "text-gray-300 hover:bg-gray-700/50"} ${
...
const ToggleSwitch = ({ enabled, setEnabled, disabled }: { enabled: boolean; setEnabled: (e: boolean) => void; disabled: boolean }) => (
  <button
    type="button"
    className={`${enabled ? "bg-indigo-600" : "bg-gray-700"} relative inline-flex items-center h-6 rounded-full w-11 transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 focus:ring-offset-gray-900 ${
      disabled ? "opacity-50 cursor-not-allowed" : ""
    }`}
